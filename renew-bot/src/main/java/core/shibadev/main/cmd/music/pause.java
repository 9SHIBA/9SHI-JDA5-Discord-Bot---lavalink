...
public class pause {
    ...
    public pause(SlashCommandInteractionEvent event, InteractionHook a) {
        if (event.getMember() != null && event.getMember().getVoiceState() != null && event.getMember().getVoiceState().getChannel() != null) {
            ...
            if (Player.CONNECTED){
                Player.pause(event.getOption("pause").getAsBoolean());

                EmbedBuilder embed = new EmbedBuilder();
                embed.setTitle("Pause Music");
                embed.addField("Pause by","<@"+event.getMember().getId()+">",true);
